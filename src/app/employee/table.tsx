...
const StudentTable = ({ data }: Props) => {
    ...
    const onEditFinish = async () => {
        const { status } = await updateStudent({ ...form.getFieldsValue(), id: selectedEmployee.id })
        if (!status) return;
    ...
    return (
        <section className="student-table">
            <PageHeader title={"Employee"} sideContent={<div className="utils">
                <Button
                    type="primary"
            ...
            <Modal
                ...
                onCancel={openAddModal ? handleCancelAddEmployee : handleCancelEditEmployee}
            >
                <Form
                    form={form}
                    name="basic"
                ...
                >
                    ...
                    >
                        <InputNumber type="number" style={{ width: "100%" }} placeholder="Enter age" />
                    </Form.Item>
